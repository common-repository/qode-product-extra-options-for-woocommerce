(function ( $ ) {
	...
	var qodefDeactivationModal = {
		init: function () {
			this.deactivationLink  = $( '#the-list' ).find( '[data-slug="qode-product-extra-options-for-woocommerce"] span.deactivate a' );
			this.deactivationModal = $( '#qode-product-extra-options-for-woocommerce-deactivation-modal' );
		...
		initSubmitFunctionality: function () {
			var submitButton = this.deactivationModal.find( '.qodef-deactivation-modal-button-submit' ),
				...
				nonceHolder  = this.deactivationModal.find( '#qode-product-extra-options-for-woocommerce-deactivation-nonce' );
						...
						$.ajax(
							{
								type: 'POST',
								data: {
									action: 'qode_product_extra_options_for_woocommerce_deactivation',
									reason: reason,
