...
class Qode_Product_Extra_Options_For_WooCommerce_Framework_WPBakery_Translator {

	public function __construct() {
		$this->add_shortcodes();

		add_filter( 'qode_product_extra_options_for_woocommerce_filter_framework_shortcode_content_html', array( $this, 'format_shortcode_content' ) );
	...
	public function map_shortcodes( $key, $shortcode ) {
		...
		if ( $shortcode->get_is_parent_shortcode() ) {
			...
			add_filter(
				'qode_product_extra_options_for_woocommerce_filter_framework_add_shortcode_container',
		...
		vc_map(
			array_merge(
				array(
					...
					'icon'        => 'qpeofw-custom-vc-icon ' . $key,
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( 'qode-product-extra-options-for-woocommerce-framework-wpbakery', QODE_PRODUCT_EXTRA_OPTIONS_FOR_WOOCOMMERCE_ADMIN_URL_PATH . '/inc/shortcodes/translators/wpbakery/assets/css/wpbakery.css' );
	}

	public function add_inline_style() {
		$shortcodes = qode_product_extra_options_for_woocommerce_framework_get_framework_root()->get_shortcodes()->get_shortcodes();
		$style      = apply_filters( 'qode_product_extra_options_for_woocommerce_filter_framework_add_wpbakery_inline_style', $style = '' );
		...
		if ( ! empty( $shortcodes ) && is_array( $shortcodes ) ) {
			...
			foreach ( $shortcodes as $key => $shortcode ) {
				...
				if ( isset( $shortcode_path ) && ! empty( $shortcode_path ) ) {
					...
					$style .= '.qpeofw-custom-vc-icon.' . $key . '{
		...
		if ( ! empty( $style ) ) {
			wp_add_inline_style( 'qode-product-extra-options-for-woocommerce-framework-wpbakery', $style );
		}
	}
}

if ( ! function_exists( 'qode_product_extra_options_for_woocommerce_framework_init_wpbakery_translator' ) ) {
	/**
	 * Function that initialize page builder module
	 */
	function qode_product_extra_options_for_woocommerce_framework_init_wpbakery_translator() {
		if ( qode_product_extra_options_for_woocommerce_is_installed( 'wpbakery' ) ) {
			new Qode_Product_Extra_Options_For_WooCommerce_Framework_WPBakery_Translator();

			if ( class_exists( 'WPBakeryShortCodesContainer' ) ) {
				$classes = apply_filters( 'qode_product_extra_options_for_woocommerce_filter_framework_add_shortcode_container', array() );
	...
	add_action( 'init', 'qode_product_extra_options_for_woocommerce_framework_init_wpbakery_translator', 99 );
}
