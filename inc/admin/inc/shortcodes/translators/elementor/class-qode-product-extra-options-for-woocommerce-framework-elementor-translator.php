...
class Qode_Product_Extra_Options_For_WooCommerce_Framework_Elementor_Translator {
	 ...
	 *
	 * @return Qode_Product_Extra_Options_For_WooCommerce_Framework_Elementor_Translator
	 */
	...
	public function generate_option_params( $shortcode ) {
		...
		if ( $shortcode->get_is_parent_shortcode() ) {
			...
			foreach ( $children as $child ) {
				$child_object = qode_product_extra_options_for_woocommerce_framework_get_framework_root()->get_shortcodes()->get_shortcode( $child );
				...
				if ( $child_object->get_is_parent_shortcode() ) {
					$shortcode_options['elements_of_child_widget']['items'][] = array(
						'field_type' => 'html',
						'name'       => 'content',
						'title'      => esc_html__( 'Content', 'qode-product-extra-options-for-woocommerce' ),
					);
				}
			}
		}
	...
	public function generate_options_array( $option_key, $option ) {
		...
		if ( ! isset( $visibility['map_for_page_builder'] ) || ( isset( $visibility['map_for_page_builder'] ) && true === $visibility['map_for_page_builder'] ) ) {
			...
			if ( 'repeater' === $option['field_type'] ) {
				$formatted_options[ $group ]['fields'][ $option_key ]['title_field'] = esc_html__( 'Item', 'qode-product-extra-options-for-woocommerce' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( 'qode-product-extra-options-for-woocommerce-framework-elementor', QODE_PRODUCT_EXTRA_OPTIONS_FOR_WOOCOMMERCE_ADMIN_URL_PATH . '/inc/shortcodes/translators/elementor/assets/css/elementor.css' );

		// Get shortcodes styles and register it during the front-end loading, scripts are enqueued on shortcodes loading.
		$shortcodes = qode_product_extra_options_for_woocommerce_framework_get_framework_root()->get_shortcodes()->get_shortcodes();
	...
	public function add_inline_style() {
		$shortcodes = qode_product_extra_options_for_woocommerce_framework_get_framework_root()->get_shortcodes()->get_shortcodes();
		$style      = apply_filters( 'qode_product_extra_options_for_woocommerce_filter_framework_add_elementor_inline_style', $style = '' );
		...
		if ( ! empty( $shortcodes ) && is_array( $shortcodes ) ) {
			...
			foreach ( $shortcodes as $key => $shortcode ) {
				...
				if ( isset( $shortcode_path ) && ! empty( $shortcode_path ) ) {
					...
					$style .= '.qpeofw-custom-elementor-icon.' . str_replace( '_', '-', $key ) . '{
		...
		if ( ! empty( $style ) ) {
			wp_add_inline_style( 'qode-product-extra-options-for-woocommerce-framework-elementor', $style );
		}
	}
	...
	public function add_elementor_widget_category( $elements_manager ) {
		$elements_manager->add_category(
			...
			array(
				'title' => esc_html__( 'QODE Product Extra Options', 'qode-product-extra-options-for-woocommerce' ),
				'icon'  => 'fa fa-plug',
			)
		);
	}
	...
	public function create_controls( $elementor_object, $shortcode_object ) {
			...
			foreach ( $control['fields'] as $field_key => $field ) {
				if ( isset( $field['field_type'] ) && 'repeater' === $field['field_type'] ) {
					...
					foreach ( $field['items'] as $item_key => $item ) {
						...
						if ( isset( $item['field_type'] ) && in_array( $item['field_type'], qode_product_extra_options_for_woocommerce_framework_elementor_get_group_types(), true ) ) {
				...
				if ( isset( $field['field_type'] ) && in_array( $field['field_type'], qode_product_extra_options_for_woocommerce_framework_elementor_get_group_types(), true ) ) {
		...
		$elementor_object->start_controls_section(
			'developer_tools',
			array(
				'label' => esc_html__( 'Developer Tools', 'qode-product-extra-options-for-woocommerce' ),
				'tab'   => \Elementor\Controls_Manager::TAB_CONTENT,
			)
		);
		...
		$elementor_object->add_control(
			'shortcode_snippet',
			array(
				'label'   => esc_html__( 'Show Shortcode Snippet', 'qode-product-extra-options-for-woocommerce' ),
				'type'    => \Elementor\Controls_Manager::SELECT,
				'default' => 'no',
				'options' => array(
					'no'  => esc_html__( 'No', 'qode-product-extra-options-for-woocommerce' ),
					'yes' => esc_html__( 'Yes', 'qode-product-extra-options-for-woocommerce' ),
				),
			)
		);
	...
	public function create_render( $shortcode_object, $params ) {
		...
		if ( isset( $params['shortcode_snippet'] ) && 'yes' === $params['shortcode_snippet'] ) {
			...
			echo qode_product_extra_options_for_woocommerce_framework_wp_kses_html( 'html', $this->get_shortcode_snippet( $shortcode_object, array_filter( $params ) ) );
		} else {

			// Handle nested shortcodes.
			if ( isset( $params['content'] ) ) {
				// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				echo qode_product_extra_options_for_woocommerce_framework_wp_kses_html( 'html', $shortcode_object->render( $params, $params['content'] ) );
			} else {
				// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				echo qode_product_extra_options_for_woocommerce_framework_wp_kses_html( 'html', $shortcode_object->render( $params ) );
			}
		}
	}
...
if ( ! function_exists( 'qode_product_extra_options_for_woocommerce_framework_get_elementor_translator' ) ) {
	/**
	 * Function that return page builder module instance
	 *
	 * @return Qode_Product_Extra_Options_For_WooCommerce_Framework_Elementor_Translator
	 */
	function qode_product_extra_options_for_woocommerce_framework_get_elementor_translator() {
		if ( qode_product_extra_options_for_woocommerce_is_installed( 'elementor' ) ) {
			return Qode_Product_Extra_Options_For_WooCommerce_Framework_Elementor_Translator::get_instance();
		}
	}
}

if ( ! function_exists( 'qode_product_extra_options_for_woocommerce_framework_init_elementor_translator' ) ) {
	/**
	 * Function that initialize page builder module
	 */
	function qode_product_extra_options_for_woocommerce_framework_init_elementor_translator() {
		qode_product_extra_options_for_woocommerce_framework_get_elementor_translator();
	}

	add_action( 'init', 'qode_product_extra_options_for_woocommerce_framework_init_elementor_translator', 1 );
}

if ( ! function_exists( 'qode_product_extra_options_for_woocommerce_framework_elementor_get_group_types' ) ) {
	function qode_product_extra_options_for_woocommerce_framework_elementor_get_group_types() {
		return array(
			'typography',
			'text_shadow',
			'box_shadow',
...
if ( ! function_exists( 'qode_product_extra_options_for_woocommerce_framework_elementor_get_tab_controls_types' ) ) {
	function qode_product_extra_options_for_woocommerce_framework_elementor_get_tab_controls_types() {
		return array(
			'start_controls_tabs',
			'start_controls_tab',
			'end_controls_tab',
