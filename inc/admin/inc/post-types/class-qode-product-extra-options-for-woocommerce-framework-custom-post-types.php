...
class Qode_Product_Extra_Options_For_WooCommerce_Framework_Custom_Post_Types {
	...
	private function set_custom_post_type( Qode_Product_Extra_Options_For_WooCommerce_Framework_Custom_Post_Type $custom_post_type ) {
	...
	public function add_custom_post_type( Qode_Product_Extra_Options_For_WooCommerce_Framework_Custom_Post_Type $custom_post_type ) {
	...
	public function register() {
		do_action( 'qode_product_extra_options_for_woocommerce_framework_action_before_post_types_register' );
		...
		do_action( 'qode_product_extra_options_for_woocommerce_framework_action_after_post_types_register' );
	}

	public function rewrite_rules( $reinit = false ) {
		$permalinks_updated = (bool) get_option( 'qode_product_extra_options_for_woocommerce_framework_permalinks_updated', false );

		if ( $reinit ) {
			flush_rewrite_rules();
		} elseif ( ! $permalinks_updated ) {
			update_option( 'qode_product_extra_options_for_woocommerce_framework_permalinks_updated', true );
	...
	public function handle_permalink_save() {
		...
		if ( isset( $_POST['permalink_structure'] ) && isset( $_POST['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), 'update-permalink' ) ) {
			...
			$permalinks      = (array) get_option( 'qode_product_extra_options_for_woocommerce_framework_permalinks', array() );
			...
			update_option( 'qode_product_extra_options_for_woocommerce_framework_permalinks', $permalinks );
