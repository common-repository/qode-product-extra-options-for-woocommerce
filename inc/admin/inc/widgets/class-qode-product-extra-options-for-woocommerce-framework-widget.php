...
abstract class Qode_Product_Extra_Options_For_WooCommerce_Framework_Widget extends WP_Widget {
	private $widget_base;
	private $widget_name;
	...
	public function import_shortcode_options( $params ) {
		...
		$framework_root = qode_product_extra_options_for_woocommerce_framework_get_framework_root();
		...
		if ( is_array( $this->widget_options ) && count( $this->widget_options ) ) { ?>
			...
			foreach ( $this->widget_options as $option ) {
				...
				if ( ! empty( $option['dependency'] ) ) {
					...
					$show     = array_key_exists( 'show', $option['dependency'] ) ? qode_product_extra_options_for_woocommerce_framework_return_widget_dependency_options_array( $instance, $option['dependency']['show'], true ) : array();
					$hide     = array_key_exists( 'hide', $option['dependency'] ) ? qode_product_extra_options_for_woocommerce_framework_return_widget_dependency_options_array( $instance, $option['dependency']['hide'] ) : array();
				...
				if ( ! empty( $option['title'] ) || ( isset( $option['name'] ) && ! empty( $option['name'] ) ) ) {
					...
					$class   = implode( ' ', $class );
					?>
					<p class="<?php echo esc_attr( $class ); ?>" <?php qode_product_extra_options_for_woocommerce_inline_attrs( $dependency_data, true ); ?>>
						...
						if ( isset( $option['name'] ) && ! empty( $option['name'] ) ) {
							...
							$field = new Qode_Product_Extra_Options_For_WooCommerce_Framework_Field_Mapper( $option );
							$field->render();
						}
						?>
			...
			<?php } ?>
		<?php } else { ?>
			<p><?php esc_html_e( 'There are no options for this widget.', 'qode-product-extra-options-for-woocommerce' ); ?></p>
			<?php
		}
	}
	...
	public function widget( $args, $instance ) {
		...
		$atts              = qode_product_extra_options_for_woocommerce_framework_map_shortcode_fields( $this->get_option_atts(), $instance );
		...
		echo qode_product_extra_options_for_woocommerce_framework_wp_kses_html( 'html', $this->render( $atts ) );
