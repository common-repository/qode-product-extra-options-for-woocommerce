(function ( $ ) {
	...
	$( document ).ready(
		function () {
			var $mainHolder      = $( '.qodef-page-v4-product-extra-options' ),
	...
	$( document ).on(
		...
		function ( event, widget ) {
			...
			if ( $widgetID && $widgetID.val().includes( 'product-extra-options' ) ) {
			...
			tabs.addClass( 'qodef--init' ).tabs(
				{
					activate: function () {
						...
						$( document.body ).trigger( 'qode_product_extra_options_for_woocommerce_trigger_tab_change' );
					}
				}
			);
		},
	...
	var qodefAdminOptionsPanel = {
		...
		initTabNavItemClick: function ( item ) {
			...
			if ( template !== null || postType !== null || post !== null || taxonomy !== null ) {
				this.setCookie(
					'qodefProductExtraOptionsActiveTab',
					panelName
				);
			...
			if ( item.length ) {
				...
				setTimeout(
					function () {
						...
						$( document.body ).on(
							'qode_product_extra_options_for_woocommerce_trigger_tab_change',
							function () {
								qodefFramework.qodefColorPicker.checkFieldPosition( $newPane );
							}
						);
		...
		setActivePanel: function () {
			var cookie 	  = this.getCookie( 'qodefProductExtraOptionsActiveTab' );
			var urlParams = new URLSearchParams( window.location.search );
			var post      = urlParams.get( 'post' );
			var postType  = urlParams.get( 'post_type' );
		...
		saveOptionsInit: function ( $adminPage ) {
			this.optionsForm = this.adminPage.find( '#qode_product_extra_options_for_woocommerce_framework_ajax_form' );
			...
			if ( this.optionsForm.length ) {
				...
				this.optionsForm.on(
					...
					function ( e ) {
						...
						var form          = $( this ),
							button_action = buttonPressed === 'qodef_save' ? 'qode_product_extra_options_for_woocommerce_action_framework_save_options_' : 'qode_product_extra_options_for_woocommerce_action_framework_reset_options_',
							ajaxData      = {
