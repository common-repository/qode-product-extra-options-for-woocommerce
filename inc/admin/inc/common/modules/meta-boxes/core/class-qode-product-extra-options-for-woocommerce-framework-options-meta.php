...
class Qode_Product_Extra_Options_For_WooCommerce_Framework_Options_Meta extends Qode_Product_Extra_Options_For_WooCommerce_Framework_Options {
	...
	public function __construct() {
		...
		add_filter( 'sanitize_post_meta_qode_product_extra_options_for_woocommerce_meta_option', array( $this, 'sanitize_meta_option' ) );
	...
	public function populate_meta_box() {
		do_action( 'qode_product_extra_options_for_woocommerce_action_framework_populate_meta_box' );
	}

	public function meta_box_register() {

		do_action( 'qode_product_extra_options_for_woocommerce_action_framework_before_meta_options_register' );
		...
		do_action( 'qode_product_extra_options_for_woocommerce_action_framework_after_meta_options_register' );
	}
	...
	public function render_meta_box( $post, $metabox ) {
		...
		qode_product_extra_options_for_woocommerce_framework_template_part( QODE_PRODUCT_EXTRA_OPTIONS_FOR_WOOCOMMERCE_ADMIN_PATH, 'inc/common', 'modules/meta-boxes/templates/holder', '', $params );
	}
	...
	public function remove_default_custom_fields() {
		$post_types = apply_filters( 'qode_product_extra_options_for_woocommerce_filter_framework_meta_box_remove', array() );
	...
	public function meta_box_save( $post_id, $post ) {
		...
		if ( is_array( $meta_boxes ) && count( $meta_boxes ) ) {
			foreach ( $meta_boxes as $meta_box ) {
				$nonces_array[] = 'qode_product_extra_options_for_woocommerce_framework_meta_box_' . $meta_box->get_slug() . '_save';
			}
		}
		...
		$post_types = apply_filters( 'qode_product_extra_options_for_woocommerce_filter_framework_meta_box_save', array( 'product' ) );
		...
		foreach ( $this->get_options() as $key => $box ) {
			$value = sanitize_meta( 'qode_product_extra_options_for_woocommerce_meta_option', array_key_exists( $key, $_POST ) ? wp_unslash( $_POST[ $key ] ) : '', 'post' );
	...
	public function enqueue_framework_meta_scripts() {
		...
		if ( function_exists( 'get_current_screen' ) && get_current_screen()->base === 'post' && in_array( get_current_screen()->post_type, $post_types, true ) ) {
			...
			do_action( 'qode_product_extra_options_for_woocommerce_action_framework_page_additional_scripts' );
		}
	}
