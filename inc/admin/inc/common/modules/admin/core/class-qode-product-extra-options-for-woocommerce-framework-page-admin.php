...
class Qode_Product_Extra_Options_For_WooCommerce_Framework_Page_Admin extends Qode_Product_Extra_Options_For_WooCommerce_Framework_Page {

	public function add_tab_element( $params ) {
		if ( isset( $params['name'] ) && ! empty( $params['name'] ) ) {
			$params['type']  = 'admin';
			$params['scope'] = $this->get_scope();
			$field           = new Qode_Product_Extra_Options_For_WooCommerce_Framework_Tab_Admin( $params );
	...
	public function add_section_element( $params ) {
		...
		if ( isset( $params['name'] ) && ! empty( $params['name'] ) ) {
			$params['type']  = 'admin';
			$params['scope'] = $this->get_scope();
			$field           = new Qode_Product_Extra_Options_For_WooCommerce_Framework_Section_Admin( $params );
			$this->add_child( $field );
	...
	public function add_row_element( $params ) {
		...
		if ( isset( $params['name'] ) && ! empty( $params['name'] ) ) {
			$params['type']  = 'admin';
			$params['scope'] = $this->get_scope();
			$field           = new Qode_Product_Extra_Options_For_WooCommerce_Framework_Row_Admin( $params );
			$this->add_child( $field );
	...
	public function add_repeater_element( $params ) {
		...
		$params['scope']         = $this->get_scope();
		$admin_option            = qode_product_extra_options_for_woocommerce_framework_get_framework_root()->get_admin_option( $this->get_scope() );
