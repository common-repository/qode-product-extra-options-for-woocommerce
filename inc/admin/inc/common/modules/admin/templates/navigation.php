...
$cpt_pages = apply_filters( 'qode_product_extra_options_for_woocommerce_filter_admin_pages_cpt', array() );
$tax_pages = apply_filters( 'qode_product_extra_options_for_woocommerce_filter_admin_pages_tax', array() );
?>
<div class="qodef-tabs-navigation-wrapper">
	<div class="qodef-tabs-navigation-wrapper-inner">
			...
			<div class="collapse navbar-collapse" id="navbar-collapse">
				<ul class="navbar-nav mr-auto">
					...
					foreach ( $pages as $page_object ) {
						...
						if ( ! empty( $dependency ) ) {
							$show     = array_key_exists( 'show', $dependency ) ? qode_product_extra_options_for_woocommerce_framework_return_dependency_options_array( $options_name, 'admin', $dependency['show'], true ) : array();
							$hide     = array_key_exists( 'hide', $dependency ) ? qode_product_extra_options_for_woocommerce_framework_return_dependency_options_array( $options_name, 'admin', $dependency['hide'] ) : array();
							$relation = array_key_exists( 'relation', $dependency ) ? $dependency['relation'] : 'and';

							$item_class[] = 'qodef-dependency-holder';
							$item_class[] = qode_product_extra_options_for_woocommerce_framework_return_dependency_classes( $show, $hide );
						...
						?>
						<li class="nav-item <?php echo esc_attr( implode( ' ', $item_class ) ); ?>" <?php qode_product_extra_options_for_woocommerce_inline_attrs( $dependency_data, true ); ?>>
		...
		<?php do_action( 'qode_product_extra_options_for_woocommerce_action_framework_before_custom_nav' ); ?>
		<?php
		$custom_nav = apply_filters( 'qode_product_extra_options_for_woocommerce_filter_framework_custom_nav', array() );
						...
						<li class="nav-item <?php echo esc_attr( $nav_item['class'] ); ?>">
							<a href="<?php echo esc_url( $nav_item['url'] ); ?>">
								...
								if ( isset( $nav_item['icon'] ) ) {
									...
									echo qode_product_extra_options_for_woocommerce_framework_wp_kses_html( 'svg', $nav_item['icon'] );
								}
								?>
