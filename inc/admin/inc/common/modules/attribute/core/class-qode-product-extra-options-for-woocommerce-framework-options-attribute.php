...
class Qode_Product_Extra_Options_For_WooCommerce_Framework_Options_Attribute extends Qode_Product_Extra_Options_For_WooCommerce_Framework_Options {
	...
	public function init_attribute_fields() {
		do_action( 'qode_product_extra_options_for_woocommerce_action_framework_custom_attribute_fields' );
	}
	...
	public function attribute_fields_display_add() {
		...
		$params['layout']      = 'div';
		qode_product_extra_options_for_woocommerce_framework_template_part( QODE_PRODUCT_EXTRA_OPTIONS_FOR_WOOCOMMERCE_ADMIN_PATH, 'inc/common', 'modules/attribute/templates/holder', '', $params );
	}
	...
	public function save_attribute_fields( $id ) {
		...
		if ( ! isset( $_POST['qode_product_extra_options_for_woocommerce_framework_attribute_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['qode_product_extra_options_for_woocommerce_framework_attribute_nonce'] ) ), 'qode_product_extra_options_for_woocommerce_framework_attribute_nonce' ) ) {
			return;
		}
		...
		foreach ( $this->get_options() as $key => $value ) {
			...
			$qode_product_extra_options_for_woocommerce_option_name = $key . '_' . strval( $id );

			if ( ( ! empty( $value ) || '0' === $value || 0 === $value ) && '' !== trim( $value ) ) {
				update_option( $qode_product_extra_options_for_woocommerce_option_name, sanitize_text_field( wp_unslash( $value ) ) );
			} else {
				delete_option( $qode_product_extra_options_for_woocommerce_option_name );
			}
		}
	}
