...
class Qode_Product_Extra_Options_For_WooCommerce_Framework_Options_Taxonomy extends Qode_Product_Extra_Options_For_WooCommerce_Framework_Options {
	public function __construct() {
		...
		add_filter( 'sanitize_term_meta_qode_product_extra_options_for_woocommerce_term_option', array( $this, 'sanitize_term_option' ) );
	...
	public function init_taxonomy_fields() {
		do_action( 'qode_product_extra_options_for_woocommerce_action_framework_custom_taxonomy_fields' );
	}
	...
	public function taxonomy_fields_display_add( $taxonomy ) {
		...
		qode_product_extra_options_for_woocommerce_framework_template_part( QODE_PRODUCT_EXTRA_OPTIONS_FOR_WOOCOMMERCE_ADMIN_PATH, 'inc/common', 'modules/taxonomy/templates/holder', '', $params );
	}
	...
	public function save_taxonomy_fields( $term_id ) {
		...
		if ( ! isset( $_POST['qode_product_extra_options_for_woocommerce_framework_taxonomy_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['qode_product_extra_options_for_woocommerce_framework_taxonomy_nonce'] ) ), 'qode_product_extra_options_for_woocommerce_framework_taxonomy_nonce' ) ) {
			return;
		}

		foreach ( $this->get_options() as $key => $value ) {
			$value = sanitize_meta( 'qode_product_extra_options_for_woocommerce_term_option', array_key_exists( $key, $_POST ) ? wp_unslash( $_POST[ $key ] ) : '', 'term' );
