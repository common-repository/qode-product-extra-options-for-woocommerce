...
class Qode_Product_Extra_Options_For_WooCommerce_Framework_Field_Repeater implements Qode_Product_Extra_Options_For_WooCommerce_Framework_Tree_Interface, Qode_Product_Extra_Options_For_WooCommerce_Framework_Child_Interface {
	...
	public function __construct( $params ) {
		...
		$this->button_text = isset( $params['button_text'] ) ? $params['button_text'] : esc_html__( '+ Add New Item', 'qode-product-extra-options-for-woocommerce' );
	...
	public function add_repeater_inner_element( $params ) {
		if ( isset( $params['name'] ) && ! empty( $params['name'] ) ) {
			...
			$field          = new Qode_Product_Extra_Options_For_WooCommerce_Framework_Field_Repeater_Inner( $params );
	...
	public function render() {
		$this->value = qode_product_extra_options_for_woocommerce_framework_get_option_value( $this->scope, $this->type, $this->name ); ?>

		<div <?php qode_product_extra_options_for_woocommerce_class_attribute( $this->params['wrapper_classes'] ); ?> <?php qode_product_extra_options_for_woocommerce_inline_attrs( $this->params['wrapper_data'] ); ?>>
			<div class="qodef-repeater-wrapper-inner">
				<div class="row">
					...
					<div class="qodef-repeater-wrapper-main col-12 <?php echo esc_attr( $this->params['sortable_class'] ); ?>" data-template="<?php echo esc_attr( str_replace( '_', '-', $this->name ) ); ?>">
								...
								<div class="qodef-repeater-fields-holder clearfix" data-index="<?php echo esc_attr( $counter ); ?>">
									...
									<div class="qodef-repeater-actions-holder">
										<div class="qodef-repeater-sort">
											<?php qode_product_extra_options_for_woocommerce_framework_svg_icon( 'expand' ); ?>
										</div>
										<div class="qodef-repeater-remove">
											<a class="qodef-clone-remove" href="#" rel="noopener noreferrer"><?php qode_product_extra_options_for_woocommerce_framework_svg_icon( 'trash' ); ?></a>
										</div>
									</div>
								</div>
								<?php
						...
						<?php
						foreach ( $this->children as $child ) {
							if ( $child instanceof Qode_Product_Extra_Options_For_WooCommerce_Framework_Field_Repeater_Inner ) {
