...
<div class="qodef-tab-wrapper <?php echo esc_attr( $class ); ?>">
	<ul class="qodef-tab-item-nav-wrapper">
		...
		foreach ( $this_object->get_children() as $child ) {
			...
			if ( ! empty( $dependency ) ) {
				$show     = array_key_exists( 'show', $dependency ) ? qode_product_extra_options_for_woocommerce_framework_return_dependency_options_array( $child->get_scope(), $child->get_type(), $dependency['show'], true ) : array();
				$hide     = array_key_exists( 'hide', $dependency ) ? qode_product_extra_options_for_woocommerce_framework_return_dependency_options_array( $child->get_scope(), $child->get_type(), $dependency['hide'] ) : array();
			...
			?>
			<li class="qodef-tab-item-nav-item <?php echo esc_attr( implode( ' ', $item_class ) ); ?>" <?php qode_product_extra_options_for_woocommerce_inline_attrs( $dependency_data, true ); ?>>
